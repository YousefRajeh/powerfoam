...
def main():
    ...
    enable_determinism()
    ck = a.ckpt or f"/home/rajehyl/gaussian_baseline_scannet/{a.scene}/ckpts/ckpt_29999_rank0.pt"
    ...
    print(f"HDBSCAN: {nclus} clusters, noise {(clus < 0).mean():.1%} of {P:,} gaussians", flush=True)
    if nclus <= 0:
    ...
    cams = np.load(f"artifacts/participation/{a.scene}_cams.npz") \
        if os.path.exists(f"artifacts/participation/{a.scene}_cams.npz") else None
    if cams is None:
