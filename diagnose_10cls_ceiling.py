...
def main():
    ...
    device = "cuda"
    scene, variant = args.scene, args.variant
    split = SCENES[scene]
    ...
    for cs in ["opengaussian19", "opengaussian15", "opengaussian10"]:
        ...
        print(f"  {cs} hard membership (current): {mi_b:.4f}/{ma_b:.4f}")
        ...
        for T in [0.001, 0.005, 0.02]:
            ...
            print(f"    soft power query T={T}: feat={mi_f:.4f}/{ma_f:.4f}  vote={mi_l:.4f}/{ma_l:.4f}")
            del simf, hist
